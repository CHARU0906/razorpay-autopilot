...
Scores every valid action with:
    EU(a) = P(success|a) * Revenue(a) - C_friction(a) - C_risk(a) - C_intervention(a)
...
def _build_reasoning(
    ...
    lines = [
        ...
        f"(EU={best.expected_utility:.2f} INR, P={best.p_success:.3f}, "
        f"delay={best.delay_h:.1f}h)",
        "Top-5 actions by EU:",
    ]
    for s in top5:
        lines.append(
            f"  {s.action_id:28s}  EU={s.expected_utility:9.2f}  "
            f"P={s.p_success:.3f}  Rev={s.revenue_inr:.2f}  "
            f"C_f={s.c_friction:.2f}  C_r={s.c_risk:.2f}  C_i={s.c_intervention:.2f}"
        )
    return "\n".join(lines)
