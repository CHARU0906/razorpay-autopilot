    ...
    >
      ...
      <div className="p-5 overflow-y-auto space-y-4 font-sans text-xs" style={{ minHeight: 0 }}>
        ...
        <div className="p-3 bg-panel/80 border border-border leading-relaxed text-muted">
          <span className="font-semibold text-text">Note — what "LLM-assisted" actually means here:</span> The
          LLM path triggers when the deterministic rules layer's confidence falls below 0.60. For this episode
          (confidence=0.55), it was triggered. In production, this step would call an LLM with the full
          failure message and context. In this benchmark demo, a deterministic keyword-matching stub is used
          as the LLM fallback (per SPEC P8 — no live LLM in the demo path). The stub found no keyword match
          in <code className="font-mono text-amber text-[10px] bg-black/40 px-1">"Do not honour"</code> and
          applied its conservative default: classify as <code className="font-mono text-green text-[10px] bg-black/40 px-1">transient</code>.
          That default happened to match ground truth. The comparison below shows why a conservative
          default outperforms confident misclassification on ambiguous volume — not why this stub is smart.
        </div>
        ...
        <div className="leading-relaxed text-text space-y-2">
          ...
          <p className="text-muted text-[11px] italic leading-normal">
            All ambiguous-population failure messages in this benchmark are deliberately uninformative
            (e.g. <code className="font-mono text-[10px]">'Do not honour'</code>), so the stub's conservative
            default is the mechanism being tested — not LLM reasoning. In production with real failure
            messages, a live LLM would have richer signal to work with. The claim here is only that
            "when in doubt, assume transient" outperforms "when in doubt, assume insufficient_funds" on
            this episode class, which the outcome comparison below demonstrates.
          </p>
        </div>
        ...
        <div className="border-t border-border pt-3 space-y-2.5 font-mono text-[11px]">
          ...
          <div className="p-3 border border-green/40 bg-green/10">
            <div className="text-green font-semibold text-[10px] uppercase tracking-wide flex items-center justify-between">
              <span>• LLM Path (Simulated Stub — conservative default)</span>
              <span className="text-[9px] text-green/80 font-normal">1h recovery</span>
            </div>
            <div className="text-muted text-[11px] mt-1 leading-normal">
              Stub found no keyword match → applied conservative default:{' '}
              <span className="text-green font-semibold">{ep.llm_inferred}</span>{' '}
              (matched Ground Truth in this case) → executed{' '}
              <code className="text-green">retry_1h</code>, recovering{' '}
              <span className="text-text font-semibold">{fmt_inr(ep.amount_inr)}</span> in 1 hour
              with 0 customer friction (<span className="text-signal-blue font-semibold">71 hours faster</span>).
              Conservative default beats misclassification on this episode class.
            </div>
          </div>
        </div>
      </div>
