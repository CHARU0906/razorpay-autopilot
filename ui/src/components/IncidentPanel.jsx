...
// Uses a simple xorshift32 seeded from the character codes of the string.
// Identical inputs always produce identical outputs — no Math.random().
...
// Outcome is DETERMINISTIC: keyed on episode_id so every replay of the same
// episode produces the same result. No Math.random() — safe for live demos.
function simulateActionAgent(episode, approved) {
  ...
  const rng = seededRandom(episode.episode_id + '_approval')
  const succeeded = approved && rng < p
  return {
